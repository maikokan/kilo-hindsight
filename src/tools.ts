/**
 * Custom tool definitions for the Hindsight Kilo plugin.
 *
 * Registers hindsight_retain, hindsight_recall, hindsight_reflect
 * as tools the agent can call explicitly.
 */

import { tool, type ToolContext } from "@kilocode/plugin/tool";
import type { ToolDefinition } from "@kilocode/plugin/tool";
  ...
  const hindsight_recall = tool({
    ...
    async execute(args, _context: ToolContext) {
      const response = await client.recall(bankId, args.query, {
        ...
        tagsMatch: config.recallTags.length ? config.recallTagsMatch as "any" | "all" | "any_strict" | "all_strict" : undefined,
      });
